...
*/
void GetTouchPoint(I2C_HandleTypeDef* hi2c,uint8_t I2Caddr,uint8_t PointNR,uint16_t* TouchPointX,uint16_t* TouchPointY,CTTouchType* TouchType,uint8_t* TouchPressure){
    uint8_t I2CBuffer[5];
	...
	*TouchPressure = I2CBuffer[3];
	//Debug
	printf("TouchPoint:%d X:%d Y%d Type:%d Pressure:%d",PointNR,*TouchPointX,*TouchPointY,*TouchType,*TouchPressure);
}